...
public interface LoginRepository3 extends JpaRepository<BoardReply, String>{
	
	@Query("SELECT m FROM BoardReply m WHERE m.member_id = :member_id")
	List<BoardReply> findByMember_id(@Param("member_id") String member_id);
	...
	Optional<BoardReply> findById(@Param("id") String id);
